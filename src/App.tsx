...
const personalData = {
  ...
  skills: {
    ...
    backend: ["Node.js", "Python", "Java", "Express", "Laravel"],
...
function ResumeDownload() {
  ...
  return (
    ...
    >
      <motion.a
        href="https://raw.githubusercontent.com/ChetanTiwari195/portfolio/6e85dc9a290f1490610481210ea76f2862b819d6/src/Assets/resume.pdf"
        download="resume.pdf"
...
function SnakeGame() {
  ...
  const getCanvasSize = () => {
    return Math.min(400, window.innerWidth - 48); // 48px for padding
  };
  ...
  const gameLoop = useCallback(() => {
    ...
    const scaledGridSize = canvasSize / tileCount;
    ...
    const newHead = {
      x: (newSnake[0].x + direction.x + tileCount) % tileCount,
      y: (newSnake[0].y + direction.y + tileCount) % tileCount,
    };
    ...
    newSnake.forEach((segment, index) => {
      ctx.beginPath();
      ctx.roundRect(
        segment.x * scaledGridSize,
        segment.y * scaledGridSize,
        scaledGridSize - 2,
        scaledGridSize - 2,
        index === 0 ? 8 : 4
      );
    ...
    ctx.beginPath();
    ctx.arc(
      food.x * scaledGridSize + scaledGridSize / 2,
      food.y * scaledGridSize + scaledGridSize / 2,
      scaledGridSize / 2 - 2,
      0,
      2 * Math.PI
    );
    ctx.fill();
  }, [snake, food, direction, gameStarted, isPaused, score, generateFood, resetGame, highScore]);
  ...
  }, [gameLoop, gameStarted, isPaused, speed]);

  return (
    <div className="bg-gray-800 p-4 sm:p-6 rounded-lg shadow-xl max-w-md mx-auto">
      <div className="flex flex-col sm:flex-row items-center justify-between mb-4 gap-2">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="text-sm">
            High Score: <span className="text-blue-400 font-bold">{highScore}</span>
          </div>
        </div>
      </div>
      ...
      >
        <canvas
          ref={canvasRef}
          className="border border-gray-700 rounded-lg mb-4 mx-auto"
        />
      </div>
      ...
      <div className="flex flex-wrap justify-center gap-4">
        <motion.button
          ...
          className="px-4 py-2 bg-blue-500 rounded-full flex items-center gap-2 hover:bg-blue-600 transition-colors"
        >
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
          onClick={resetGame}
          className="px-4 py-2 bg-red-500 rounded-full flex items-center gap-2 hover:bg-red-600 transition-colors"
        >
      ...
      <div className="mt-4 text-sm text-gray-400 text-center">
        Use arrow keys or swipe to control the snake. Collect the red dots to grow and increase your score!
      </div>
    </div>
  );
}
      ...
      <div className="flex items-center justify-center flex-col lg:flex-row">
        ...
        >
          ...
          <motion.div
            ...
            transition={{ delay: 0.2 }}
          >
            <h1 className="text-5xl font-bold mb-4">Chetan Tiwari</h1>
      ...
      <footer className="bg-gray-900 py-8">
        <div className="container mx-auto px-4 text-center">
          <p className="text-gray-400">
            © 2025 Chetan Tiwari. All rights reserved.
          </p>
        </div>
      </footer>
    </div>
